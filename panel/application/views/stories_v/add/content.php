            ...
            <?php foreach ($settings as $key => $value) : ?>
                <div class="tab-pane fade <?= ($key == 0 ? 'show active' : null) ?>" id="lang-<?= $value->lang ?>" role="tabpanel" aria-labelledby="lang-<?= $value->lang ?>-tab">
                    <div class="form-group">
                        ...
                        <input class="form-control form-control-sm rounded-0" placeholder="Hikaye Adı [Dil : <?=$value->lang?>]" name="title[<?=$value->lang?>]" required>
                    </div>
                    <div class="form-group">
                        <label>Hikaye URL</label>
                        <input class="form-control form-control-sm rounded-0" placeholder="Hikaye URL [Dil : <?=$value->lang?>]" name="url[<?=$value->lang?>]">
                    </div>
                    <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                        <div class="input-group-prepend">
                            <span class="input-group-text">Hikaye Kapak Görseli Seçiniz [Dil : <?=$value->lang?>]</span>
                        </div>
