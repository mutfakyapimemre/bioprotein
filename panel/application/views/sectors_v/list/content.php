...
<script>
	$(document).ready(function() {
		$(document).on("click", ".createSectorBtn", function(e) {
			...
			$('#sectorModal').iziModal('destroy');
			createModal("#sectorModal", "Yeni Sektör Ekle", "Yeni Sektör Ekle", 600, true, "20px", 0, "#e20e17", "#fff", 1040, function() {
				$.post(url, {}, function(response) {
					$("#sectorModal .iziModal-content").html(response);
					TinyMCEInit();
					flatPickrInit();
					$(".tagsInput").select2({
						width: 'resolve',
			...
			$("#sectorModal").iziModal("setFullscreen", false);
		});
		...
		$(document).on("click", ".updateSectorBtn", function(e) {
			...
			$('#sectorModal').iziModal('destroy');
			let url = $(this).data("url");
			createModal("#sectorModal", "Sektör Düzenle", "Sektör Düzenle", 600, true, "20px", 0, "#e20e17", "#fff", 1040, function() {
				$.post(url, {}, function(response) {
					$("#sectorModal .iziModal-content").html(response);
					TinyMCEInit();
					flatPickrInit();
