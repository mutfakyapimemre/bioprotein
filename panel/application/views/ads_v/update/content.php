...
<form id="updateHomeAds" onsubmit="return false" method="post" enctype="multipart/form-data">
    <div class="mb-3 nav-tabs-horizontal">
        ...
        <div class="tab-content" id="myTabContent">
            <?php foreach ($settings as $key => $value) : ?>
                ...
                <div class="tab-pane fade <?= ($key == 0 ? 'show active' : null) ?>" id="lang-<?= $lang ?>" role="tabpanel" aria-labelledby="lang-<?= $lang ?>-tab">
                    ...
                    <div class="form-group">
                        <label>Link</label>
                        <input class="form-control form-control-sm rounded-0" placeholder="Link [Dil : <?= $lang ?>]" name="url[<?= $lang ?>]" value="<?= (!empty($item->url->$lang) ? $item->url->$lang : null); ?>" required>
                    </div>
                    <div class="row">
                        <div class="col-3">
                            <picture>
        ...
        <button role="button" data-url="<?= base_url("ads/update/$item->id"); ?>" class="btn btn-sm btn-outline-primary rounded-0 btnUpdate">Güncelle</button>
