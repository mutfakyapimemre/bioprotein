...
<div class="rs-breadcrumbs">
    <img src="<?= base_url("public/images/bg/contact-bg.webp") ?>" alt="<?= strto("lower|upper", $product->title->$lang) ?>">
    <div class="breadcrumbs-inner">
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center">
                    <h1 class="page-title"><?= strto("lower|upper", $product->title->$lang) ?></h1>
                    <ul>
                        ...
                        <li><a class="active" rel="dofollow" href="<?= base_url($languageJSON["routes"]["urunler"]); ?>" title="<?= strto("lower|upper", $languageJSON["routes"]["urunler"]); ?>"><?= strto("lower|upper", $languageJSON["detailPages"]["products"]); ?></a></li>
                        <li><?= strto("lower|upper", $product->title->$lang) ?></li>
                    </ul>
                </div>
            </div>
        </div>
...
<div class="shop-single-page-area sec-spacer2">
    <div class="container">
        <div class="row">
            <div class="col-12 col-sm-12 col-md-12 col-lg-8 col-xl-8">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                            <div class="inner-single-product-slider">
                                <div class="inner">
                                    <div class="slider single-product">
                                        <?php foreach ($product_images as $k => $v) : ?>
                                            <?php if ($v->product_id == $product->id && $v->isCover) : ?>
                                                <div>
                                                    <div class="images-single"> <img src="<?= get_picture("products_v", $v->url) ?>" alt="<?= $product->title->$lang ?>" class="img-fluid w-100"> </div>
                                                </div>
                                            <?php endif ?>
                                        <?php endforeach ?>
                                        <?php foreach ($product_images as $k => $v) : ?>
                                ...
                                <div class="slider single-product-nav mx-3">
                                    <?php foreach ($product_images as $k => $v) : ?>
                                        <?php if ($v->product_id == $product->id && $v->isCover) : ?>
                                            <div class="images-slide-single">
                                                <img src="<?= get_picture("products_v", $v->url) ?>" alt="<?= $product->title->$lang ?>" style="min-height:75px;" class="img-fluid w-100">
                                            </div>
                                        <?php endif ?>
                                    <?php endforeach ?>
                                    <?php foreach ($product_images as $k => $v) : ?>
                ...
                <div class="tab-btm">
                    <div class="d-flex justify-content-between">
                        <h2 class="flex-fill"><?= $product->title->$lang ?></h2>
                        <div class="flex-shrink-1">
                            <div class="product-btn mt-0">
                                <a rel="dofollow" target="_blank" href="<?= !empty($product->external_url->$lang) ? $product->external_url->$lang : "javascript:void(0)" ?>"><?= $languageJSON["detailPages"]["externalURL"] ?></a>
                            </div>
                        </div>
                    </div>
                    <h4 class="cat">
                        <strong class="text-secondary"><?= $languageJSON["detailPages"]["productPrice"] ?></strong>
                        <?php if ($product->isDiscount) : ?>
                            <span class="product-price-mark"><?= $formatter->formatCurrency($product->price->$lang, $currency) ?></span>
                            <span class="product-price"><?= $formatter->formatCurrency((($product->price->$lang) - (($product->price->$lang * $product->discount->$lang) / 100)), $currency) ?></span>
                        <?php else : ?>
                            <span class="product-price"><?= $formatter->formatCurrency($product->price->$lang, $currency) ?></span>
                        <?php endif ?>
                    </h4>


                    <small class="date d-block"><small><?= iconv("ISO-8859-9", "UTF-8", strftime("%d %B %Y, %A %X", strtotime($product->updatedAt))); ?></small></small>
                    <p class="cat">
                        <strong><?= $languageJSON["detailPages"]["productCategories"] ?>:</strong>
                        <?php $count = count($product_categories) ?>
                        <?php $i = 1 ?>
                        <?php foreach ($product_categories as $key => $value) : ?>
                            <?php if ($i < $count) : ?>
                                <a rel="dofollow" href="<?= base_url($languageJSON["routes"]["urunler"] . "/{$value->seo_url->$lang}") ?>" title="<?= $value->title->$lang ?>"><?= $value->title->$lang ?></a>,
                            <?php else : ?>
                                <a rel="dofollow" href="<?= base_url($languageJSON["routes"]["urunler"] . "/{$value->seo_url->$lang}") ?>" title="<?= $value->title->$lang ?>"><?= $value->title->$lang ?></a>
                            <?php endif ?>
                            <?php $i++ ?>
                                    ...
                                    <div class="post-item">
                                        <div class="post-img">
                                            <a rel="dofollow" href="<?= base_url($languageJSON["routes"]["urunler"] . "/" . $languageJSON["routes"]["urun"] . "/{$value->url->$lang}") ?>" title="<?= $value->title->$lang ?>">
                                                <?php $imgURL = null ?>
                                                <?php if (!empty($product_all_images)) : ?>
                                                    <?php foreach ($product_all_images as $k => $v) : ?>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="section-title text-center">
                        ...
                        <h2><?= $languageJSON["detailPages"]["suggestedProducts"] ?></h2>
                    </div>
                </div>
            </div>
